...
func main() {
	...
	var contStr string = "xcxxc"
	fmt.Printf("contStr=%v\n", strings.Count(contStr, "c"))
	//不区分大小写的字符串比较(==是区分字母大小写的): fmt.Println(strings.EqualFold("abc", "Abc")) // true
	fmt.Println("EqualFold", strings.EqualFold("abc", "Abc")) // true
	//返回子串在字符串第一次出现的 index 值，如果没有返回-1 : strings.Index("NLT_abc", "abc") // 4
	fmt.Println("Index", strings.Index("NLT_abc", "abc")) // 4
	//返回子串在字符串最后一次出现的 index，如没有返回-1 : strings.LastIndex("go golang", "go")
	fmt.Println("LastIndex", strings.LastIndex("go golang", "go"))
	//将指定的子串替换成 另外一个子串: strings.Replace("go go hello", "go", "go 语言", n)
	//n 可以指 定你希望替换几个，如果 n=-1 表示全部替换
	fmt.Println("Replace", strings.Replace("go go hello", "go", "go 语言", 1))
	//按照指定的某个字符，为分割标识，将一个字符串拆分成字符串数组:
	testA := strings.Split("hello,wrold,ok", ",")
	fmt.Println("testA", testA) // true
	//将字符串的字母进行大小写的转换: strings.ToLower("Go") // go strings.ToUpper("Go") // GO
	fmt.Println("ToLower", strings.ToLower("GGEW")) //
	fmt.Println("ToUpper", strings.ToUpper("1Gw"))  //
	//将字符串左右两边的空格去掉: strings.TrimSpace(" tn a lone gopher ntrn ")
	fmt.Println("TrimSpace", strings.TrimSpace(" tn a lone gopher ntrn ")) //
	...
	fmt.Println("HasPrefix", strings.HasPrefix("ftp://192.168.10.1", "ftp")) //
	//判断字符串是否以指定的字符串结束: strings.HasSuffix("NLT_abc.jpg", "abc") //false
	fmt.Println("HasSuffix", strings.HasSuffix("NLT_abc.jpg", "abc")) //
}
